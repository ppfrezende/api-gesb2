...
export async function generateInvoiceToCustomerPDF(
  request: FastifyRequest,
  reply: FastifyReply,
) {
  const generateInvoiceToCustomerQuerySchema = z.object({
    interventionId: z.string(),
    invoiceToCustomerId: z.string(),
  });
  8;

  const { interventionId, invoiceToCustomerId } =
    generateInvoiceToCustomerQuerySchema.parse(request.params);
  ...
  try {
    ...
    const getInvoiceToCustomerUseCase = makeGetInvoiceToCustomerUseCase();

    const { intervention } = await getInterventionUseCase.execute({
      interventionId,
    });
    const { invoiceToCustomer } = await getInvoiceToCustomerUseCase.execute({
      invoiceToCustomerId,
    });
    ...
    const {
      ...
      invoiceToCustomer!.isDolarInvoice,
      invoiceToCustomer!.invoice_currency_quote,
    );
    ...
    generateHeader(doc, invoiceToCustomer!.isDolarInvoice);
    doc
      .lineCap('butt')
