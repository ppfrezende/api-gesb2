...
export function generateTimesheetResumingTable(
  ...
  const calculatedDayHoursArray = intervention.timesheets.flatMap((timesheet) =>
    timesheet.timesheetdays.map((day) => {
      ...
      const dates = formatWeekday(day.day, isDolarInvoice ? 'USD' : 'BRL');
  ...
  const timesheetResumingTable = {
    headers: [
      ...
      {
        label: `${intervention.isOffshore ? 'OFFSHORE' : 'ONSHORE'}`,
        align: 'center',
      },
