...
export interface TimeSheetDaysRepository {
  findByTechnicianId(technician_id: string): Promise<TimeSheetDay[] | null>;
  findByTimeSheetDataId(
    timeSheetDataId: string,
  ): Promise<TimeSheetDay[] | null>;
  listMany(page: number): Promise<TimeSheetDay[]>;
  createMany(data: Prisma.TimeSheetDayUncheckedCreateInput[]): Promise<void>;
